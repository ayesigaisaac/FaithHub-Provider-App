    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-faith-slate">
            {primaryLabel}
          </p>
          <p className="mt-1 whitespace-nowrap text-[clamp(1.45rem,2.8vw,2rem)] font-black leading-none tracking-[-0.02em] text-faith-ink dark:text-slate-100">
            {primaryValue}
          </p>
        </div>
        {primaryMeta ? (
          <p className="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
            {primaryMeta}
          </p>
        ) : null}
      </div>
      ...
      <div className="mt-3 divide-y divide-faith-line/70 rounded-xl border border-faith-line/60 dark:divide-slate-800 dark:border-slate-800">
        {secondary.map((item) => (
          <div key={item.label} className="flex items-center justify-between gap-3 px-3 py-2.5">
            <span className="text-[12px] text-faith-slate">{item.label}</span>
            <span className="text-[15px] font-semibold tabular-nums text-faith-ink dark:text-slate-100">
              {item.value}
            </span>
          </div>
        ))}
