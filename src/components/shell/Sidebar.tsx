    ...
    <aside className="flex h-full w-full flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      <div className="flex items-center justify-between border-b border-slate-200 px-5 py-5 dark:border-slate-800">
        ...
        <button
          type="button"
          onClick={onClose}
          className="inline-flex rounded-[12px] p-2 text-slate-500 transition-colors hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 lg:hidden"
        ...
        <div className="space-y-5">
          {sidebarSections.map((section) => (
            <section key={section.id}>
              <h2 className="mb-2 px-2 text-[11px] font-semibold tracking-[0.14em] text-slate-400 dark:text-slate-500">{section.title}</h2>
