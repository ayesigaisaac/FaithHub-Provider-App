...
function MetricTile({ card }: { card: MetricCard }) {
  const toneStyle =
    card.tone === "brand"
      ...
      : card.tone === "accent"
        ...
        : undefined;
  const baseCls =
    card.tone === "brand" || card.tone === "accent"
      ? "ring-0 shadow-sm"
      : card.tone === "good"
  ...
  return (
    <div className={cn("rounded-3xl p-4 shadow-sm transition", baseCls)} style={toneStyle}>
      <div className={cn("text-[11px] font-black uppercase tracking-widest", card.tone === "brand" || card.tone === "accent" ? "text-white/85" : "text-slate-500 dark:text-slate-400")}>
        {card.label}
      </div>
      <div className={cn("mt-2 text-2xl font-extrabold tracking-tight", card.tone === "brand" || card.tone === "accent" ? "text-white" : "text-slate-900 dark:text-slate-50")}>
        {card.value}
      </div>
      <div className={cn("mt-1 text-[11px] leading-relaxed", card.tone === "brand" || card.tone === "accent" ? "text-white/90" : "text-slate-500 dark:text-slate-400")}>
        {card.hint}
      </div>
    </div>
  );
