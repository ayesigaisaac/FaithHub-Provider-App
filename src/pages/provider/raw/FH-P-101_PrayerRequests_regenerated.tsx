    ...
    <div className="rounded-[28px] border border-slate-200 bg-white p-4 transition-colors">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {subtitle ? (
            <div className="mt-1 text-[11px] text-slate-500 leading-5">{subtitle}</div>
          ) : null}
        </div>
  ...
  return (
    <div className="rounded-[26px] border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="text-[12px] uppercase tracking-wide text-slate-500 font-semibold">{label}</div>
          <div className="mt-2 text-[42px] leading-none font-black tracking-tight text-slate-900">{value}</div>
  ...
  return (
    <div className="rounded-[28px] border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          <div className="text-[12px] font-semibold uppercase tracking-wide text-slate-500">Prayer destination preview</div>
      ...
      <div className={cx("mt-4 rounded-[24px] border border-slate-200 bg-slate-50 p-3", mode === "mobile" ? "max-w-[320px]" : "") }>
...
function TemplateTile({ template }: { template: TemplateCard }) {
  return (
    <div className="rounded-[22px] border border-slate-200 bg-white p-3">
      ...
      <div className="mt-3 text-[13px] font-bold text-slate-900">{template.title}</div>
      <div className="mt-1 text-[12px] text-slate-500 leading-5">{template.subtitle}</div>
      <button
        type="button"
    ...
    <div className="rounded-[22px] border border-slate-200 bg-white px-3 py-3">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="mt-1 text-[12px] text-slate-500">{lead.role} · {lead.location}</div>
        </div>
...
function PrayerRequestsPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#f2f2f2] text-slate-900">
      <div className="mx-auto w-full max-w-[1600px] px-5 py-5">
        ...
        <div className="mt-4 flex items-center justify-between rounded-full border border-slate-200 bg-white px-4 py-2 text-[13px] text-slate-500">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.55fr_0.95fr_0.7fr]">
          <div className="space-y-4">
            <Card
              ...
              right={<div className="flex items-center gap-2 text-[12px] text-slate-500"><span className="rounded-full border border-slate-200 px-3 py-1">{filteredRequests.length} requests</span><span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-amber-700">3 due today</span></div>}
            >
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative min-w-[280px] flex-1">
                  ...
                  <input
                    ...
                    className="w-full rounded-[20px] border border-slate-200 bg-slate-50 px-11 py-3 text-[13px] outline-none focus:border-emerald-300 focus:bg-white"
                  />
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  {[
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-2">
                {ROUTING_LANES.map((lane) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <div className="text-[14px] font-bold text-slate-900">{lane.title}</div>
                        <div className="mt-1 text-[12px] text-slate-500">{lane.subtitle}</div>
                      </div>
                      <div className="grid h-7 min-w-[28px] place-items-center rounded-full border border-amber-300 bg-white px-2 text-[12px] font-bold text-amber-700">
                        {lane.count}
                      </div>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="mt-4 text-[48px] leading-[0.98] font-black tracking-tight text-slate-900">
                {selectedRequest.title}
              </div>
              <div className="mt-3 text-[14px] leading-7 text-slate-600">{selectedRequest.summary}</div>

              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-3 py-3">
                  <div className="text-[11px] uppercase tracking-wide text-slate-400">Routing</div>
                  <div className="mt-1 text-[13px] font-semibold text-slate-900">Care lead: {selectedRequest.owner}</div>
                  <div className="mt-1 text-[12px] text-slate-500">Source: {selectedRequest.source}</div>
                </div>
                <div className="rounded-[20px] border border-slate-200 bg-slate-50 px-3 py-3">
                  <div className="text-[11px] uppercase tracking-wide text-slate-400">Follow-up</div>
                  <div className="mt-1 text-[13px] font-semibold text-slate-900">Received: {selectedRequest.submittedLabel}</div>
                  <div className="mt-1 text-[12px] text-slate-500">{selectedRequest.followUpLabel}</div>
                </div>
              </div>

              <div className="mt-4 space-y-2">
                {selectedRequest.notes.map((note) => (
                  <div key={note} className="rounded-[18px] border border-slate-200 bg-white px-3 py-2.5 text-[12px] text-slate-600">
                    {note}
                  </div>
                ))}
              </div>
            ...
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between rounded-[18px] border border-emerald-300 bg-emerald-50 px-3 py-2.5">
                  <span className="text-[13px] font-semibold text-slate-900">Answered prayers ready for testimonies</span>
                ...
                <div className="flex items-center justify-between rounded-[18px] border border-amber-300 bg-amber-50 px-3 py-2.5">
                  <span className="text-[13px] font-semibold text-slate-900">Prayer wall health</span>
                ...
                <div className="flex items-center justify-between rounded-[18px] border border-rose-300 bg-rose-50 px-3 py-2.5">
                  <span className="text-[13px] font-semibold text-slate-900">Counseling bridge</span>
      ...
      {previewOpen ? (
        <div className="fixed inset-0 z-[90]">
          ...
          <div className="absolute inset-y-0 right-0 flex w-full max-w-[760px] flex-col border-l border-slate-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <div>
                <div className="text-[15px] font-bold text-slate-900">Prayer request preview</div>
                <div className="mt-1 text-[12px] text-slate-500">Expanded preview for the selected request.</div>
              </div>
              <button
                type="button"
                onClick={() => setPreviewOpen(false)}
                className="grid h-10 w-10 place-items-center rounded-2xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-5 py-3">
              <button
                ...
                className={cx(
                  ...
                  previewMode === "desktop" ? "bg-[#1d2b64] text-white" : "bg-slate-100 text-slate-600",
                )}
              >
                Desktop
              </button>
              <button
                ...
                className={cx(
                  "rounded-full px-4 py-1.5 text-[12px] font-semibold",
                  previewMode === "mobile" ? "bg-[#1d2b64] text-white" : "bg-slate-100 text-slate-600",
                )}
              >
                Mobile
              </button>
            ...
            <div className="flex-1 overflow-auto p-5">
              <div className={cx("mx-auto rounded-[34px] border border-slate-200 bg-slate-50 p-4", previewMode === "mobile" ? "max-w-[360px]" : "max-w-[680px]") }>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-[22px] border border-slate-200 bg-white p-4">
                    ...
                    <div className="mt-1 text-[13px] text-slate-500">{selectedRequest.group} · {selectedRequest.source}</div>
                  </div>
                  <div className="rounded-[22px] border border-slate-200 bg-white p-4">
                    <div className="text-[12px] font-semibold uppercase tracking-wide text-slate-500">Prayer activity</div>
                    <div className="mt-2 text-[16px] font-bold text-slate-900">{selectedRequest.prayerCount} prayers</div>
                    <div className="mt-1 text-[13px] text-slate-500">{selectedRequest.followUpLabel}</div>
                  </div>
                </div>
              </div>
            </div>
